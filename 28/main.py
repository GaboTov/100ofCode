from tkinter import * 
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"   
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None
# ---------------------------- TIMER RESET ------------------------------- # 
def reset_timer ():
    global reps
    window.after_cancel(timer)
    title.config(text="Timer")
    canvas.itemconfig(txt_canvas,text="00:00")
    check.config(text="")
    reps = 0
# ---------------------------- TIMER MECHANISM -------------------- ----------- # 
def start_timer():
    global reps
    reps += 1
    work = WORK_MIN * 60
    short_break = SHORT_BREAK_MIN * 60
    long_break = LONG_BREAK_MIN * 60 
    if reps%8 == 0:
        title.config(fg=PINK, text="Long Break")
        count_down(long_break)
        
    elif reps%2 == 0 :
        title.config(fg=PINK, text="Break")
        count_down(short_break)
        
    else:
        title.config(fg=GREEN, text="Timer")
        count_down(work)
        
    
    

        
# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
window = Tk()
def count_down(count):
    count_min = math.floor(count / 60)
    count_sec = count%60
    if count_sec < 10:
       count_sec = f"0{count_sec}"
    if count_min < 10:
       count_min = f"0{count_min}"
    canvas.itemconfig(txt_canvas,text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down,count -1)
    else:
        start_timer()
        work_sessions = math.floor(reps/2)
        if reps%2 == 0:
            str_check = ["✔️" for _ in range(work_sessions)]
            check.config(text=f"{''.join(str_check)}")
# ---------------------------- UI SETUP ------------------------------- #





window.title("Pomodoro app")
window.config(padx=100,pady=100, bg=YELLOW)
title = Label(text="Timer", bg=YELLOW,fg=GREEN, font=(FONT_NAME, 40, "bold"))
title.grid(row=1,column=2)
canvas = Canvas(width= 205, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file="28/tomato.png")
canvas.create_image(100,112,image=tomato_img)
txt_canvas = canvas.create_text(103,130, text="00:00",fill="white",font=(FONT_NAME, 35, "bold"))
canvas.grid(row=2,column=2)

start_btn = Button(text="Start", bg="white",highlightthickness=0,command=start_timer )
start_btn.grid(row=3, column=1)
reset_btn = Button(text="Reset", bg="white", highlightthickness=0, command=reset_timer)
reset_btn.grid(row=3, column=3)
check = Label(bg=YELLOW, fg=GREEN, font=(FONT_NAME,18))

check.grid(row=4,column=2)


window.mainloop()